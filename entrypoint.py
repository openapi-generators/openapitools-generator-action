...
(_, generator, docker_repository, generator_tag, openapi_file, openapi_url, config_file, template_dir, *args) = argv

cmd = f"docker run -u {getuid()}:1001 --rm --workdir /github/workspace -v {getenv('GITHUB_WORKSPACE')}:/github/workspace"
cmd = f"{cmd} {docker_repository}/openapitools/openapi-generator-cli:{generator_tag} generate"
